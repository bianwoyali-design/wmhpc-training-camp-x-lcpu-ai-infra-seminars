...
def main():
    ...
    meta = dict(
        ...
        sources={
            p.name: digest(p)
            for p in [ROOT / "probe.py", ROOT / "run.py", ROOT / "reference.py"]
        },
    ...
    try:
        for pattern in ["weak", "correlated"]:
            for logit in [-4.0, 0.0, 4.0]:
                ...
                init = initial.cuda().bfloat16()
                final = torch.empty_like(init)
                output = torch.empty_like(v)
                ws = torch.empty(
                    ext.get_workspace_size(16, 3, 1), device="cuda", dtype=torch.uint8
                )
                ext.fwd(
                    q,
                    k,
                ...
                for h, seed in enumerate(seeds):
                    ...
                    assert torch.equal(ref[0], ref[1]), (
                        "No checkpoint rounding precedes first-chunk outputs"
                    )
                    ...
                    rows.append(
                        dict(
                            ...
                            beta_logit=logit,
                            seed=seed,
                            output_relative=error["relative_l2"],
                            output_max_abs=error["max_abs"],
                            inverse_relative=(
                                (inverse[h] - exact).norm() / exact.norm()
                            ).item(),
                            inverse_residual=(
                                ((eye + lower) @ inverse[h] - eye).norm() / eye.norm()
                            ).item(),
                            lower_power8_norm=torch.linalg.matrix_norm(
                                torch.linalg.matrix_power(lower, 8)
                            ).item(),
                            state_rounding_output_relative=0.0,
                            finite=error["finite"],
                        )
                    )
        csvwrite(out / "inverse_probe.csv", rows)
