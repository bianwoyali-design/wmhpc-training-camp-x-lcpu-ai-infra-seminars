...
@torch.jit.script
def recurrence(q:torch.Tensor,k:torch.Tensor,v:torch.Tensor,g:torch.Tensor,beta:torch.Tensor,
               initial:torch.Tensor,points:List[int]):
    ...
    snapshots=torch.empty((3,len(points),q.size(1),k.size(2),v.size(2)),dtype=q.dtype)
    decay=g.exp();cursor=0
    for t in range(q.size(0)):
        ...
        state=state+(beta[t].unsqueeze(-1)*k[t]).unsqueeze(0).unsqueeze(-1)*residual.unsqueeze(-2)
...
def activate(q,k,v,g,beta,initial,alog,bias):
    q=q.double();k=k.double()
    q=q/(q.square().sum(-1,keepdim=True)+1e-6).sqrt()
    k=k/(k.square().sum(-1,keepdim=True)+1e-6).sqrt()
    gate=-5*torch.sigmoid(alog.double().exp()[None,:,None]*(g.double()+bias.double()[None]))
    return q*128**-.5,k,v.double(),gate,torch.sigmoid(beta.double()),initial.double().transpose(-1,-2).contiguous()
...
def validate():
    ...
    for seed in [0,1,2]:
        for t in [1,15,16,17,33]:
            ...
            q=torch.randn(t,2,128).double();k=torch.randn_like(q)
            q=torch.nn.functional.normalize(q,dim=-1);k=torch.nn.functional.normalize(k,dim=-1)
            v=torch.randn_like(q);g=-torch.rand_like(q)*.1;beta=torch.sigmoid(torch.randn(t,2).double())
            initial=torch.randn(2,128,128).double()*.1
            out,states=recurrence(q*128**-.5,k,v,g,beta,initial,[t])
            gold,final=naive.naive_recurrent_kda(q[None],k[None],v[None],g[None],beta[None],
                                               initial_state=initial[None],output_final_state=True)
            oe=((out[0]-gold[0]).norm()/gold.norm()).item()
            se=((states[0,0]-final[0]).norm()/final.norm()).item()
            assert max(oe,se)<2e-5,(seed,t,oe,se)
            rows.append(dict(seed=seed,length=t,output_relative=oe,state_relative=se))
    return rows
