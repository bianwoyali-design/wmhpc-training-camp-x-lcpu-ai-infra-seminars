...
CASES = {
    "h12_n1": (12, [8192], False),
    "h12_n8": (12, [1024] * 8, False),
    "h96_n1": (96, [8192], False),
    "h96_n8": (96, [1024] * 8, False),
...
METRICS = (
    [
        ...
        "sm__ops_path_tensor_op_hmma_src_fp16_dst_fp16.sum",
        "l1tex__data_bank_conflicts_pipe_lsu_mem_shared_op_ld.sum",
        "l1tex__data_bank_conflicts_pipe_lsu_mem_shared_op_st.sum",
    ]
    + [
        f"sm__inst_executed_pipe_{pipe}.avg.pct_of_peak_sustained_elapsed"
        for pipe in ["alu", "lsu", "adu", "xu", "fma"]
    ]
    + [
        f"smsp__warp_issue_stalled_{kind}_per_warp_active.pct"
        for kind in [
            "short_scoreboard",
            "long_scoreboard",
            "barrier",
...
def inputs(case):
    ...
    n = len(lengths)
    t = sum(lengths) if varlen else lengths[0]
    b = 1 if varlen else n
    torch.manual_seed(1234)
    dims = (b, t, h, 128)
    q = torch.nn.functional.normalize(
        torch.randn(dims, device="cuda"), dim=-1
    ).bfloat16()
    k = torch.nn.functional.normalize(
        torch.randn(dims, device="cuda"), dim=-1
    ).bfloat16()
    v = torch.randn(dims, device="cuda", dtype=torch.bfloat16)
    ...
    init = (0.1 * torch.randn(n, h, 128, 128, device="cuda")).bfloat16()
    final = torch.empty_like(init)
    out = torch.empty_like(v)
    workspace = torch.empty(
        ext.get_workspace_size(sum(lengths), h, n), device="cuda", dtype=torch.uint8
    )
    cu = (
        torch.tensor(
...
def target(case, path):
    ...
    if path:
        path.write_text(
            json.dumps(
                dict(case=case, repeat_output_exact=True, repeat_state_exact=True)
            )
            + "\n"
        )
...
def normal_timing(case, out):
    ...
    spec = importlib.util.spec_from_file_location(
        "parallelism_helpers", PREVIOUS / "run.py"
    )
...
def parse_raw(path):
    ...
    return [
        ...
        if "__" in name and value not in ("", None)
    ]
...
def report(out):
    ...
    for raw in sorted(out.glob("*.raw.csv")):
        label = raw.name.removesuffix(".raw.csv")
        case, cache = label.rsplit("__", 1)
        rows = parse_raw(raw)
        groups = collections.defaultdict(dict)
        for r in rows:
            name = r["Kernel Name"]
            stage = (
                "k1" if "_prepare" in name else "k2" if "_recurrence" in name else None
            )
            if stage is None:
                continue
            normalized.append(
            ...
            try:
                groups[stage][r["Metric Name"]] = float(
                    r["Metric Value"].replace(",", "")
                )
            except ValueError:
                pass
        ...
        for stage, m in groups.items():
            for key in METRICS:
                if key not in m:
                    raise RuntimeError(f"{label}/{stage}: missing metric {key}")
            duration = m["gpu__time_duration.sum"]
            seconds = duration * 1e-9
            dram = m["dram__bytes_read.sum"] + m["dram__bytes_write.sum"]
            flop = (
                m["sm__ops_path_tensor_op_hmma_src_bf16_dst_fp32.sum"]
                + m["sm__ops_path_tensor_op_hmma_src_fp16_dst_fp16.sum"]
            )
            summaries.append(
                dict(
                    case=case,
                    ...
                    l2_pct=m["lts__throughput.avg.pct_of_peak_sustained_elapsed"],
                    l1_pct=m["l1tex__throughput.avg.pct_of_peak_sustained_elapsed"],
                    tensor_elapsed_pct=m[
                        "sm__pipe_tensor_cycles_active.avg.pct_of_peak_sustained_elapsed"
                    ],
                    tensor_active_pct=m[
                        "sm__pipe_tensor_cycles_active.avg.pct_of_peak_sustained_active"
                    ...
                    eligible=m["smsp__warps_eligible.avg.per_cycle_active"],
                    ...
                    dram_bytes=dram,
                    dram_GBs=dram / seconds / 1e9,
                    tensor_ops=flop,
                    tensor_TFLOPs=flop / seconds / 1e12,
                    tensor_ops_per_dram_byte=flop / dram if dram else "",
                    clock_MHz=m["gpc__cycles_elapsed.avg.per_second"] / 1e6,
                    short_scoreboard_pct=m[
                        "smsp__warp_issue_stalled_short_scoreboard_per_warp_active.pct"
                    ],
                    long_scoreboard_pct=m[
    ...
    write_csv(out / "metrics.csv", normalized)
    ...
    for s in summaries:
        lines.append(
            "| "
            + " | ".join(
                str(s[k]) if isinstance(s[k], str) else f"{s[k]:.2f}"
                for k in [
                    "case",
                    "cache",
                    "stage",
...
def reproduce(args):
    ...
    meta = dict(
        ...
        metrics=METRICS,
        clock_control="none",
        source_sha256=digest(ROOT / "run.py"),
        extension=flash_kda_C.__file__,
        extension_sha256=digest(flash_kda_C.__file__),
        helper_sha256=digest(PREVIOUS / "run.py"),
        commands=[],
    )
    ...
    with (out / "clocks.csv").open("w") as clocklog:
        ...
        try:
            for case in names:
                normal += normal_timing(case, out)
                write_csv(out / "normal_timing.csv", normal)
                for cache in caches:
                    ...
                    cmd = [
                        ...
                        "none",
                        "--cache-control",
                        "all" if cache == "cold" else "none",
                        "--replay-mode",
                        "kernel" if cache == "cold" else "application",
                        "--section",
                        "LaunchStats",
                        "--section",
                        "Occupancy",
...
def main():
    ...
    p.add_argument("command", choices=["target", "reproduce", "report"])
    ...
    if args.command == "target":
        if not args.case:
            p.error("target requires --case")
        target(args.case, args.out)
    elif args.command == "report":
        if args.out is None:
            p.error("report requires --out")
        report(args.out)
    else:
        reproduce(args)
