...
def relative(x, y):
    return (
        torch.linalg.vector_norm((x - y).double())
        / torch.linalg.vector_norm(y.double()).clamp_min(1e-30)
    ).item()
...
def validate():
    ...
    for seed in range(3):
        ...
        g = -0.1 * torch.rand(B, T, H, K)
        beta = torch.sigmoid(torch.randn(B, T, H))
        state = 0.1 * torch.randn(B, H, K, V)
        gold, final = naive(
        ...
        def record(name, o, s, expected):
            ...
            if expected:
                assert max(oe, se) < 2e-5, (name, oe, se)
            else:
                assert max(oe, se) > 0.01, (name, oe, se)
            rows.append(
                dict(
                    seed=seed,
                    test=name,
                    expected_equal=expected,
                    output_relative_error=oe,
                    state_relative_error=se,
                )
            )
        ...
        for parts in [2, 4, 8]:
            ...
            for j in range(parts):
                ...
                o, s = naive(
                    ...
                    g,
                    beta,
                    initial_state=state[..., sl],
                    output_final_state=True,
                )
                outputs.append(o)
                states.append(s)
            record(
                f"value_split_{parts}",
                torch.cat(outputs, -1),
                torch.cat(states, -1),
                True,
            )
        for heads in [1, 2]:
            ...
            for j in range(0, H, heads):
                ...
                states.append(s)
            record(
                f"head_group_{heads}", torch.cat(outputs, 2), torch.cat(states, 1), True
            )
        for carry in [False, True]:
            ...
            for j in [0, 16]:
                o, s = naive(
                    ...
                    g[:, j : j + 16],
                    beta[:, j : j + 16],
                    initial_state=s if carry or j == 0 else None,
                    output_final_state=True,
                )
                outputs.append(o)
            record(
                "time_split_carry" if carry else "time_split_reset",
                torch.cat(outputs, 1),
                s,
                carry,
            )
        ...
        for j in [0, 64]:
            ...
            states.append(s)
        record(
            "key_split_without_reduction", sum(outputs), torch.cat(states, -2), False
        )

        # Merely stacking heads' A matrices while sharing one B is not batched GEMM.
        a = torch.randn(4, 16, 128)
        b = torch.randn(4, 128, 16)
        true = a @ b
        wrong = (a.reshape(64, 128) @ b[0]).reshape(4, 16, 16)
        error = relative(wrong, true)
        assert error > 0.1
        rows.append(
            dict(
                seed=seed,
                test="head_stack_shared_B",
        ...
        gg = g.double()
        a = (
            torch.eye(K) - bb[..., None, None] * kk[..., :, None] * kk[..., None, :]
        ) * gg.exp()[..., None, :]
