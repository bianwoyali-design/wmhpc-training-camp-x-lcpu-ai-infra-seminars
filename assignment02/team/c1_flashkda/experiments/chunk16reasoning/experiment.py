...
class Kernels:
    def __init__(self, library):
        ...
        signatures = {
            ...
            'launch_config': [ct.c_void_p, ct.c_void_p, ct.c_int, ct.c_int, ct.c_int, ct.c_void_p],
            'resources_config': [ct.c_int, ct.c_int, ct.POINTER(ct.c_int)],
            'launch_decay': [ct.c_void_p, ct.c_void_p, ct.c_int, ct.c_int, ct.c_int, ct.c_void_p],
            'launch_mma': [ct.c_void_p, ct.c_void_p, ct.c_void_p, ct.c_int, ct.c_int, ct.c_int, ct.c_int, ct.c_void_p],
        }
    ...
    def decay(self, gates, out, mode):
        self.invoke('launch_decay', gates.data_ptr(), out.data_ptr(), gates.shape[0],
                    gates.shape[1], mode, torch.cuda.current_stream().cuda_stream)

    def mma(self, a, b, out, c, stage, warps):
        self.invoke('launch_mma', a.data_ptr(), b.data_ptr(), out.data_ptr(), a.shape[0],
                    c, stage, warps, torch.cuda.current_stream().cuda_stream)
    ...
    def resources(self, warps):
        ...
        for c in CHUNKS:
            for w in warps:
                ...
                rows.append(dict(chunk=c, warps=w, registers_per_thread=a[0], shared_bytes=a[1],
                                 local_bytes_per_thread=a[2], max_blocks_per_sm=a[3]))
        return rows
...
def configure():
    ...
    if uuid != 'unknown' and not uuid.startswith('GPU-'):
        uuid = 'GPU-' + uuid
    return dict(device=p.name, device_uuid=uuid,
                multiprocessors=p.multi_processor_count, torch=torch.__version__, cuda=torch.version.cuda)
...
def inverse_checks(kernels, settings):
    ...
    for seed in settings['seeds']:
        ...
        k = torch.nn.functional.normalize(torch.randn(n, 64, 128, device='cuda'), dim=-1)
        s = (-.1 * torch.rand_like(k)).cumsum(1)
        beta = torch.sigmoid(torch.randn(n, 64, device='cuda'))
        cases = {
            'structured': torch.tril(((k*s.exp()) @ (k*(-s).exp()).transpose(-1, -2))*beta[:, :, None], -1),
            'zero': torch.zeros(n, 64, 64, device='cuda'),
            'stress': torch.tril(.2*torch.randn(n, 64, 64, device='cuda'), -1),
        }
        for case, full in cases.items():
            for c in CHUNKS:
                ...
                a = eye + l.double()
                ref = torch.linalg.solve_triangular(a, eye, upper=False, unitriangular=True)
                ...
                for method in methods(c, settings['warps']):
                    ...
                    if method.startswith('unified'):
                        if warp_ref is None:
                            ...
                            raise AssertionError((case, c, method, 'warp outputs differ'))
                    res = (torch.linalg.matrix_norm(a@x-eye)/math.sqrt(c)).max().item()
                    if case == 'zero' and not torch.equal(x, eye):
                        raise AssertionError('L=0 should produce exact identity')
                    if case == 'structured' and res >= .02:
                        raise AssertionError((c, method, res))
                    rows.append(dict(seed=seed, case=case, chunk=c, method=method, max_residual=res,
                        max_relative_error=(torch.linalg.matrix_norm(x-ref)/torch.linalg.matrix_norm(ref)).max().item(),
                        max_abs_difference_from_torch_fp16=(x-model).abs().max().item()))
    return rows


def gate_cases(seed, n):
    gen = torch.Generator().manual_seed(seed)
    cases = {f'constant_{g:g}': torch.full((n, 64), g) for g in (0., -.1, -1., -3., -5.)}
    cases['uniform'] = -5*torch.rand(n, 64, generator=gen)
    cases['mixed'] = torch.where(torch.rand(n, 64, generator=gen) < .1, -5., -.01)
    return cases
        ...
        for name, full in gate_cases(seed, settings['accuracy_batch']).items():
            ...
            for c in CHUNKS:
                ...
                for mode, method in enumerate(DECAY_METHODS):
                    ...
                    rel = ((torch.linalg.vector_norm(x-ref, dim=1)/torch.linalg.vector_norm(ref, dim=1)).max().item()
                           if finite else None)
                    if method in ('tile16', 'direct') and (not finite or rel >= .02):
                    ...
                    if name == 'constant_-5' and method == 'center' and c == 32 and not finite:
                        raise AssertionError('centered C=32 should be finite')
                    rows.append(dict(seed=seed, case=name, chunk=c, method=method, elements=x.numel(),
                                     zero_count=int((x==0).sum()), inf_count=int(torch.isinf(x).sum()),
                                     nan_count=int(torch.isnan(x).sum()), max_relative_error=rel,
                                     max_abs_error=(x-ref).abs().max().item() if finite else None))
    return rows
    ...
    for seed in settings['seeds']:
        ...
        for c in CHUNKS:
            for stage, name in enumerate(STAGES):
                m, n, k = shape(stage, c)
                a = torch.randn(settings['accuracy_batch'], m, k, device='cuda').bfloat16()
                b = torch.randn(settings['accuracy_batch'], k, n, device='cuda').bfloat16()
                ...
                for w in settings['warps']:
                    ...
                    rel = (torch.linalg.matrix_norm(x.double()-ref)/torch.linalg.matrix_norm(ref)).max().item()
                    if not torch.isfinite(x).all() or rel > 1e-5:
                        raise AssertionError((name, c, w, rel))
                    rows.append(dict(seed=seed, stage=name, chunk=c, warps=w, m=m, n=n, k=k,
                                     max_relative_error=rel, max_abs_error=(x.double()-ref).abs().max().item()))
    return rows


def checks(kernels, settings, suites):
    functions = {'inverse': inverse_checks, 'range': range_checks, 'mma': mma_checks}
    return {f'{suite}_accuracy': functions[suite](kernels, settings) for suite in suites}
...
def timing_cases(kernels, settings, suite):
    ...
    for tokens in settings['tokens']:
        ...
        for c in CHUNKS:
            ...
            if suite == 'inverse':
                l = torch.tril(.02*torch.randn(batch, c, c, device='cuda'), -1).half().float().contiguous()
                out = torch.empty_like(l)
                for method in methods(c, settings['warps']):
                    cases.append(TimingCase(dict(**common, method=method), partial(kernels.inverse, l, out, method)))
            elif suite == 'range':
                # Mild gates keep all baselines numerically valid during timing.
                gates = -.1*torch.rand(batch, c, device='cuda')
                out = torch.empty(batch, c, c, device='cuda')
                for mode, method in enumerate(DECAY_METHODS):
                    cases.append(TimingCase(dict(**common, method=method), partial(kernels.decay, gates, out, mode)))
            else:
                for stage, name in enumerate(STAGES):
                    m, n, k = shape(stage, c)
                    ...
                    for w in settings['warps']:
                        info = dict(**common, stage=name, method=f'wmma_w{w}', m=m, n=n, k=k,
                                    wmma_per_matrix=(m//16)*(n//16)*(k//16), padding_fraction=0)
                        cases.append(TimingCase(info, partial(kernels.mma, a, b, out, c, stage, w)))
    return cases
...
def benchmark(kernels, settings, suite, output, device_uuid):
    ...
    command = ['nvidia-smi', '--query-gpu=timestamp,uuid,clocks.sm,power.draw,temperature.gpu', '--format=csv', '-lms', '200']
        ...
        for round_id in range(settings['rounds']):
            ...
            for case, graph in order:
                ...
                for _ in range(settings['samples']):
                    ...
                    samples.append(start.elapsed_time(end)*1000/settings['graph_launches'])
                us = statistics.median(samples)
                rows.append(dict(round=round_id, **case.info, median_us=us, min_us=min(samples), max_us=max(samples),
                                 ns_per_token=us*1000/(case.info['batch']*case.info['chunk'])))
    finally:
