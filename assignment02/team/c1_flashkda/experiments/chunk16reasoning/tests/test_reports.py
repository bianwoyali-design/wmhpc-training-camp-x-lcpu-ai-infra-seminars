...
from report import aggregate, render, best_table


class Reports(unittest.TestCase):
    def test_best_candidate_does_not_use_official_or_other_stage(self):
        rows=[]
        for stage,c,method,us in [('gram',16,'wmma_w1',10),('gram',16,'official',1),
                                  ('gram',32,'wmma_w4',40),('projection',16,'wmma_w1',100)]:
            rows.append(dict(workload='single',chunk=str(c),stage=stage,method=method,median_us=str(us)))
    ...
    def test_cli_without_torch(self):
        ...
        args=module.parser().parse_args(['reproduce','--suite','inverse','--warps','8,1,4','--quick'])
