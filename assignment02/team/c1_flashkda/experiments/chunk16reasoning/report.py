...
def aggregate(rows, dimensions):
    ...
    return {
        k: (statistics.median(v), min(v), max(v), len(v)) for k, v in groups.items()
    }
...
def timing_table(rows, stage=False):
    ...
    lines = [
        "| "
        + " | ".join(dims + ["mode", "µs/call", "round range µs", "rounds"])
        + " |",
        "|" + "|".join(["---"] * (len(dims) + 4)) + "|",
    ]
    for key, (med, mn, mx, n) in sorted(groups.items()):
        lines.append(
            "| " + " | ".join(key) + f" | {med:.3f} | {mn:.3f}–{mx:.3f} | {n} |"
        )
    return lines
...
def best_table(rows, stage=False):
    ...
    lines = [
        "| " + " | ".join(head + ["best candidate", "µs/call", "vs C=16"]) + " |",
        "|" + "|".join(["---"] * (len(head) + 3)) + "|",
    ]
...
def render(directory):
    ...
    for suite in ["range", "inverse", "mma"]:
        ...
        lines += [f"## {suite}", ""]
        if suite != "range":
            lines += best_table(rows, stage=suite == "mma") + [""]
        ...
        if suite == "range":
            ...
            for r in accuracy:
                if r["case"] == "constant_-5":
                    ...
                    lines.append(
                        f"| {r['chunk']} | {r['method']} | {r['zero_count']} | {r['inf_count']} | {r['nan_count']} | {err} |"
                    )
        else:
            ...
            for r in accuracy:
                key = (
                    r.get("case", r.get("stage")),
                    r["chunk"],
                    r.get("method", r.get("warps")),
                )
                grouped[key].append(float(r[field]))
            lines += [
                "",
                f"正确性指标：{field}，下表跨种子取最大值。",
                "",
                "| input/stage | CHUNK | method/warps | error |",
                "|---|---:|---|---:|",
            ]
            for key, values in sorted(grouped.items()):
                lines.append("| " + " | ".join(key) + f" | {max(values):.6g} |")
        lines += [""]
    if not present:
    ...
    for f in sorted(p.glob("*clocks.csv")):
        samples = read(f)
        frequencies = [
            float(r["clocks.current.sm [MHz]"].split()[0])
            for r in samples
            if r.get("clocks.current.sm [MHz]", "").split()[0:1]
            and r["clocks.current.sm [MHz]"].split()[0].isdigit()
        ]
        if frequencies:
            lines.append(
                f"- {f.name}: {len(frequencies)} samples, {min(frequencies):g}–{max(frequencies):g} MHz。"
            )
    lines += [
        f"- racecheck: {meta.get('racecheck', '见原始日志；没有日志不能视为通过')}。",
        "",
        "## 解释边界",
        "",
