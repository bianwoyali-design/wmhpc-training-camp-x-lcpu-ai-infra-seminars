...
def build(force=False):
    ...
    for required in [
        FLASH / "cutlass/include/cute/tensor.hpp",
        FLASH / "csrc/smxx/utils.cuh",
    ]:
        if not required.exists():
            raise RuntimeError(f"缺少构建依赖：{required}；见 README 的环境要求。")
    sources = [
        ROOT / "csrc" / name
        for name in ("official.cu", "unified.cu", "decay.cu", "mma.cu")
    ]
    dependencies = sources + [
        FLASH / "csrc/smxx/utils.cuh",
        FLASH / "cutlass/include/cutlass/bfloat16.h",
        FLASH / "cutlass/include/cute/arch/mma_sm80.hpp",
    ]
    flags = [
        "-O2",
        "-std=c++17",
    ...
    provenance = dict(
        ...
        hashes={
            ...
            else str(f.relative_to(FLASH)): digest(f)
            for f in dependencies
        },
    )
    commit = subprocess.run(
        ["git", "-C", str(FLASH / "cutlass"), "rev-parse", "HEAD"],
        capture_output=True,
        text=True,
    )
    provenance["cutlass_commit"] = (
    ...
    if (
        ...
        and LIBRARY.exists()
        and stamp.exists()
        and json.loads(stamp.read_text()) == provenance
    ):
...
def inspect_instructions(destination):
    ...
    with (BUILD / "inspect.log").open("w") as log:
        subprocess.run(
            [*base, "-ptx", str(source), "-o", str(BUILD / "mma.ptx")],
            stdout=log,
            stderr=log,
            check=True,
        )
        subprocess.run(
            [*base, "-cubin", str(source), "-o", str(BUILD / "mma.cubin")],
            stdout=log,
            stderr=log,
            check=True,
        )
        with (BUILD / "mma.sass").open("w") as sass:
            subprocess.run(
                [disasm, "-c", str(BUILD / "mma.cubin")],
                stdout=sass,
                stderr=log,
                check=True,
            )
    ptx = (BUILD / "mma.ptx").read_text()
    sass = (BUILD / "mma.sass").read_text()
    ...
    evidence = dict(
        ...
        sass_opcodes=opcodes,
        static_hmma_count=sass.count("HMMA.16816.F32.BF16"),
...
def validate_range(output):
    ...
    with (output / "device.log").open("w") as log:
        subprocess.run(
            [str(binary), str(output / "inputs.txt"), str(output)],
            stdout=log,
            stderr=log,
            check=True,
        )
    ...
    (output / "provenance.json").write_text(
        json.dumps(
            dict(
                ...
                nvcc=subprocess.check_output([nvcc, "--version"], text=True),
                source_hashes={
                    name: digest(ROOT / name) for name in ("numerics.py", "csrc/ftz.cu")
                },
            ),
            indent=2,
        )
...
def settings(args):
    return dict(
        ...
        accuracy_batch=4 if args.quick else 8,
        graph_launches=10 if args.quick else 100,
        rounds=2 if args.quick else 5,
        samples=2 if args.quick else 5,
        tokens=[None, 8192] if args.quick else [None, 8192, 8192 * 96],
        quick=args.quick,
    )
    ...
    for cmd in ("check", "bench", "reproduce"):
        q = subs.add_parser(
            ...
            help={
                "check": "只运行正确性检查",
                "bench": "正确性检查 + 计时 + 汇总",
                "reproduce": "构建 + racecheck + 正确性 + 计时 + 汇总",
            }[cmd],
        )
        q.add_argument("--suite", choices=["all", *SUITES], default="all")
        q.add_argument("--warps", type=parse_warps, default=[1, 4, 8])
        q.add_argument(
            "--quick", action="store_true", help="缩小检查/采样规模；仅用于冒烟测试"
        )
        if cmd != "check":
            q.add_argument(
                "--out", type=Path, help="新结果目录；必须不存在，防止覆盖历史记录"
            )
        if cmd == "reproduce":
            q.add_argument(
                "--skip-racecheck",
                action="store_true",
                help="显式跳过 sanitizer，metadata 会记录",
            )
...
def main():
    ...
    if args.command == "validate":
        validate_range(
            ...
            or ROOT
            / "runs"
            / datetime.now(timezone.utc).strftime("ftz-%Y%m%dT%H%M%S.%fZ")
        )
        return
    ...
    if args.command == "build":
        ...
        return
    provenance = build()
    from experiment import Kernels, benchmark, checks, configure, write_csv
    ...
    if args.command == "check":
        ...
        print(
            json.dumps(
                dict(status="passed", rows={k: len(v) for k, v in data.items()}),
                indent=2,
            )
        )
        return
    if (
        ...
        and not args.skip_racecheck
        and not shutil.which("compute-sanitizer")
    ):
        raise RuntimeError(
            "找不到 compute-sanitizer；安装 CUDA 工具或明确使用 --skip-racecheck。"
        )
    output = (
        args.out
    ...
    meta = dict(
        ...
        **device,
        ...
        source_hashes={
            ...
            for f in [*ROOT.glob("*.py"), *(ROOT / "csrc").glob("*.cu")]
        },
        racecheck="not_requested",
        timing="preallocated, hot reused inputs, CUDA Graph launches, randomized configuration order",
    )
    ...
    try:
        ...
        if "mma" in suites:
            meta["instruction_evidence"] = inspect_instructions(
        ...
        for name, rows in checks(kernels, cfg, suites).items():
            write_csv(output / f"{name}.csv", rows)
        if "inverse" in suites:
            write_csv(output / "inverse_resources.csv", kernels.resources(cfg["warps"]))
        for suite in suites:
            print(f"Timing {suite} ...", flush=True)
            write_csv(
                output / f"{suite}_timing.csv",
                benchmark(kernels, cfg, suite, output, device["device_uuid"]),
            )
