...
def build():
    ...
    launch = (
        (OFFICIAL / "csrc/smxx/fwd_launch.cu")
        .read_text()
        .split("// Explicit instantiations")[0]
    )
    wrapper = r"""
    ...
    for parts in [0, 1, 2, 4]:
        ...
        if parts:
            k = replace(
                ...
                f"constexpr int kComputeThreads = {128 // parts};",
            )
            k = replace(
                k,
                "const int warp_id = compute_tid / 32;",
                f"const int warp_id = compute_tid / 32 + blockIdx.z * {4 // parts};",
            )
            k = replace(
                k, "warp_role, kComputeThreads, 1", "warp_role, kComputeThreads, 32"
            )
            k = replace(
                k,
...
"""
            ...
            l = replace(
                ...
                f"constexpr int kK2Threads = 64 + {128 // parts};",
            )
            l = replace(l, "dim3 grid_k2(N, H);", f"dim3 grid_k2(N, H, {parts});")
            l = replace(
                l,
                "out_ptr, T_total, H, N, cu_seqlens_ptr, total_tiles",
                "out_ptr, static_cast<BF16*>(final_state_ptr), T_total, H, N, cu_seqlens_ptr, total_tiles",
            )
        (directory / "fwd_kernel2.cuh").write_text(k)
        source = directory / "launch.cu"
        source.write_text(l + wrapper)
        cmd = [
            "nvcc",
            "-O3",
            "-std=c++17",
            ...
            "-I" + str(OFFICIAL / "csrc"),
            "-I" + str(OFFICIAL / "csrc/smxx"),
            "-I" + str(OFFICIAL / "cutlass/include"),
            "-I" + str(OFFICIAL / "cutlass/tools/util/include"),
            str(source),
            "-o",
            str(directory / "kernel.so"),
        ]
        stamp = hashlib.sha256((k + l + wrapper + str(cmd)).encode()).hexdigest()
        if (
            not (directory / "stamp").exists()
            or (directory / "stamp").read_text() != stamp
        ):
            print("Building", parts, flush=True)
            with (directory / "build.log").open("w") as f:
