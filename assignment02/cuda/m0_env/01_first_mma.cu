...
__global__ void mma_demo(const __half *A, const __half *B, float *D) {
  ...
  ah[0] =
      __halves2half2(A[(group) * 16 + tig * 2], A[(group) * 16 + tig * 2 + 1]);
  ...
  bh[0] =
      __halves2half2(B[(tig * 2) * 8 + group], B[(tig * 2 + 1) * 8 + group]);
...
int main() {
  ...
  for (int r = 0; r < 16; r++)
    for (int k = 0; k < 16; k++)
      hA[r * 16 + k] = __float2half((r + k) % 5 - 2);
  for (int k = 0; k < 16; k++)
    for (int n = 0; n < 8; n++)
      hB[k * 8 + n] = __float2half((k * n) % 3 - 1);
  for (int r = 0; r < 16; r++)
    for (int n = 0; n < 8; n++)
      for (int k = 0; k < 16; k++)
        ref[r * 8 + n] +=
            __half2float(hA[r * 16 + k]) * __half2float(hB[k * 8 + n]);
