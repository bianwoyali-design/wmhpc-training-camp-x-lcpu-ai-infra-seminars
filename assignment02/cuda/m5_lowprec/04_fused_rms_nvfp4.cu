...
static int best_rms_block(int M, int K) {
  static const std::unordered_map<uint64_t, int> table = {
      {shape_key(1, 4096), 512},    {shape_key(16, 4096), 512},
      {shape_key(256, 4096), 512},  {shape_key(1024, 4096), 128},
      {shape_key(4096, 4096), 128}, {shape_key(16384, 4096), 128},
      {shape_key(4096, 7168), 128}, {shape_key(16384, 7168), 128},
      {shape_key(4096, 8192), 256}, {shape_key(16384, 8192), 128},
  };

  auto it = table.find(shape_key(M, K));
  return it != table.end() ? it->second : 256;
}
...
template <int BLOCK>
__global__ void
fused_kernel(const __nv_bfloat16 *__restrict__ in,
             const __nv_bfloat16 *__restrict__ w, uint8_t *__restrict__ dataOut,
             uint8_t *__restrict__ sfOut, int M, int K, float eps) {
  __shared__ float red[BLOCK / 32];
...
#pragma unroll
    ...
    __syncthreads();
    const float rnorm = red[0];
    for (int g = threadIdx.x; g < groupsPerRow; g += BLOCK) {
      const int k0 = g * NVFP4_GROUP;
      float vals[NVFP4_GROUP], amax = 0.f;
      {
        const float4 raw0 = *reinterpret_cast<const float4 *>(xr + k0),
                     raww0 = *reinterpret_cast<const float4 *>(w + k0);
        const auto *h0 = reinterpret_cast<const __nv_bfloat162 *>(&raw0),
                   *hw0 = reinterpret_cast<const __nv_bfloat162 *>(&raww0);
#pragma unroll
        for (int i = 0; i < 4; ++i) {
          const float2 f = __bfloat1622float2(h0[i]),
                       fw = __bfloat1622float2(hw0[i]);
          const float y0 = f.x * rnorm * fw.x, y1 = f.y * rnorm * fw.y;
          vals[i * 2] = y0;
          vals[i * 2 + 1] = y1;
          amax = fmaxf(amax, fabsf(y0));
          amax = fmaxf(amax, fabsf(y1));
        }
      }
      {
        const float4 raw1 = *reinterpret_cast<const float4 *>(xr + k0 + 8),
                     raww1 = *reinterpret_cast<const float4 *>(w + k0 + 8);
        const auto *h1 = reinterpret_cast<const __nv_bfloat162 *>(&raw1),
                   *hw1 = reinterpret_cast<const __nv_bfloat162 *>(&raww1);
#pragma unroll
        for (int i = 0; i < 4; ++i) {
          const float2 f = __bfloat1622float2(h1[i]),
                       fw = __bfloat1622float2(hw1[i]);
          const float y0 = f.x * rnorm * fw.x, y1 = f.y * rnorm * fw.y;
          vals[8 + i * 2] = y0;
          vals[8 + i * 2 + 1] = y1;
          amax = fmaxf(amax, fabsf(y0));
      ...
      sfOut[sf_swizzled_offset(row, g, numKTiles)] = sf8.__x;
      const size_t outBase =
          (size_t)row * (K / 2) + (size_t)g * (NVFP4_GROUP / 2);
#pragma unroll
      for (int i = 0; i < NVFP4_GROUP; i += 2) {
        const __nv_fp4x2_e2m1 q(make_float2(vals[i] * inv, vals[i + 1] * inv));
        dataOut[outBase + i / 2] = q.__x;
      }
    }
    __syncthreads();
  }
...
static void launch_fused(const __nv_bfloat16 *in, const __nv_bfloat16 *w,
                         ...
                         float eps, int sms) {
  if (M <= 16) {
    fused_kernel<128><<<M, 128>>>(in, w, dataOut, sfOut, M, K, eps);
  } else if (M == 256 && K == 4096) {
    fused_kernel<256><<<M, 256>>>(in, w, dataOut, sfOut, M, K, eps);
  } else {
    fused_kernel<128><<<M, 128>>>(in, w, dataOut, sfOut, M, K, eps);
  }
}
...
static void launch_two_step(const __nv_bfloat16 *in, const __nv_bfloat16 *w,
  ...
  const int grid = M;
  const int block = best_rms_block(M, K);
  switch (block) {
  case 128:
    rms_norm_baseline_kernel<128><<<grid, 128>>>(in, w, mid, M, K, eps);
    break;
  case 256:
