...
static int swizzle_128B(int row, int colByte) {
  row &= 0x7;
  const int chunk = (colByte >> 4) ^ row;
  return (row << 7) + (chunk << 4) + (colByte & 0xf);
}
static int swizzle_64B(int row, int colByte) {
  const int base = row >> 2;
  row &= 0x3;
  const int chunk = (colByte >> 4) ^ row;
  return (base << 8) + (row << 6) + (chunk << 4) + (colByte & 0xf);
}
static int swizzle_32B(int row, int colByte) {
  const int base = row >> 1;
  row &= 0x1;
  const int chunk = (colByte >> 4) ^ row;
  return (base << 6) + (row << 5) + (chunk << 4) + (colByte & 0xf);
}
