...
// 下面的 poly_eval_global 把 8 个多项式系数放在 global memory，每个线程读 8
// 次。 它保留不动，作为对比基准。 任务：
...
static float run_one(poly_fn fn, const char *name, const float *d_x, float *d_y,
  ...
  CUDA_CHECK_KERNEL();
  CUDA_CHECK(
      cudaMemcpy(h_y, d_y, (size_t)n * sizeof(float), cudaMemcpyDeviceToHost));
...
int main() {
  ...
  float h_coef[8] = {1.f,     -0.5f,     0.25f,     -0.125f,
  ...
  CUDA_CHECK(
      cudaMemcpy(d_coef, h_coef, sizeof(h_coef), cudaMemcpyHostToDevice));
